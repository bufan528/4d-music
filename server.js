...
// LLM 客户端单例：只使用服务端配置的 key 和 baseURL，杜绝客户端覆盖导致的 SSRF/内存泄漏
let llmClient = null;
function getLLMClient() {
    if (!llmClient && DEFAULT_API_KEY) {
        llmClient = new OpenAI({ apiKey: DEFAULT_API_KEY, baseURL: DEFAULT_BASE_URL, timeout: 30000 });
    }
    return llmClient;
}

const callLLM = async (ctx, scores) => {
    // [安全加固] 完全使用服务端配置，不接受任何客户端传入的 key/base/model
    if (!DEFAULT_API_KEY || DEFAULT_API_KEY.length < 5) return "AI建议服务未连接。";

    const client = getLLMClient();
    if (!client) return "AI建议服务未连接。";
...
const initReferenceLibrary = async () => {
    ...
    for (const song of SONGS_METADATA) {
        ...
        if (fs.existsSync(p)) {
            const tmp = path.join(__dirname, 'processed', `ref_${song.id}.wav`);
            try {
...
app.get('/version', (req, res) => {
    res.send(`Backend v${pkg.version} (Security + Async Init + LLM Pool Optimized) is running!`);
});
...
app.post('/analyze', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        if (analysisResult.aligned && analysisResult.scoreResult) {
            ...
            responseData.scores = {
                pitch: scoreResult.details.pitch,                                    // 音准
                rhythm: aligned.rhythmScore,                                          // 节奏
                stability: scoreResult.details.stability,                             // 稳定性
                tone: Math.min(100, Math.round(analysisResult.features.resonance.head * 100)), // 音色/头腔共鸣
                tension: Math.min(100, Math.round(analysisResult.features.resonance.chest * 100)), // 张力/胸腔支撑
                technique: scoreResult.details.technique,                             // 技巧/颤音
                articulation: scoreResult.details.articulation,                       // 咬字/清晰度
                range: scoreResult.details.range,                                     // 音域
                emotion: scoreResult.details.emotion,                                 // 情感表现力
